    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        protected virtual string ProductListToHtmlTable(Order order, string languageId, string vendorId)
        {
            ...
            for (int i = 0; i <= table.Count - 1; i++)
            {
                ...
                sb.AppendLine("<td style=\"padding: 0.6em 0.4em;text-align: left;\">" + System.Net.WebUtility.HtmlEncode(productName));
                //add download link
                ...
                if (_catalogSettings.ShowSkuOnProductDetailsPage)
                {
                    ...
                    {
                        sb.AppendLine("<br />");
                        sb.AppendLine(string.Format(_localizationService.GetResource("Messages.Order.Product(s).SKU", languageId), System.Net.WebUtility.HtmlEncode(sku)));
                    }
                }
                sb.AppendLine("</td>");
            ...
            if (String.IsNullOrEmpty(vendorId))
            {
                ...
                //gift cards
                var _servicegiftCard = EngineContextExperimental.Current.Resolve<IGiftCardService>();                
                var gcuhC = _servicegiftCard.GetAllGiftCardUsageHistory(order.Id); 
                foreach (var gcuh in gcuhC)
                {
                    var giftCard = EngineContextExperimental.Current.Resolve<IGiftCardService>().GetGiftCardById(gcuh.GiftCardId);
                    string giftCardText = String.Format(_localizationService.GetResource("Messages.Order.GiftCardInfo", languageId), System.Net.WebUtility.HtmlEncode(giftCard.GiftCardCouponCode));
        ...
        protected virtual string RecentlyViewedProductsListToHtmlTable(Customer customer, string languageId, bool withPicture)
        {
            ...
            var recentlyViewedProductsService = EngineContextExperimental.Current.Resolve<IRecentlyViewedProductsService>();
            var pictureService = EngineContextExperimental.Current.Resolve<IPictureService>();
        ...
        public virtual void AddOrderTokens(IList<Token> tokens, Order order, string languageId, string vendorId = "")
        {
            ...
            tokens.Add(new Token("Order.BillingStateProvince", !String.IsNullOrEmpty(order.BillingAddress.StateProvinceId) ? EngineContextExperimental.Current.Resolve<IStateProvinceService>().GetStateProvinceById(order.BillingAddress.StateProvinceId).GetLocalized(x => x.Name) : ""));
            tokens.Add(new Token("Order.BillingZipPostalCode", order.BillingAddress.ZipPostalCode));
            tokens.Add(new Token("Order.BillingCountry", !String.IsNullOrEmpty(order.BillingAddress.CountryId) ? EngineContextExperimental.Current.Resolve<ICountryService>().GetCountryById(order.BillingAddress.CountryId).GetLocalized(x => x.Name) : ""));
            ...
            tokens.Add(new Token("Order.ShippingStateProvince", order.ShippingAddress != null && !String.IsNullOrEmpty(order.ShippingAddress.StateProvinceId) ? EngineContextExperimental.Current.Resolve<IStateProvinceService>().GetStateProvinceById(order.ShippingAddress.StateProvinceId).GetLocalized(x => x.Name) : ""));
            tokens.Add(new Token("Order.ShippingZipPostalCode", order.ShippingAddress != null ? order.ShippingAddress.ZipPostalCode : ""));
            tokens.Add(new Token("Order.ShippingCountry", order.ShippingAddress != null && !String.IsNullOrEmpty(order.ShippingAddress.CountryId) ? EngineContextExperimental.Current.Resolve<ICountryService>().GetCountryById(order.ShippingAddress.CountryId).GetLocalized(x => x.Name) : ""));
            ...
            if (customValues != null)
            {
                ...
                {
                    sbCustomValues.AppendFormat("{0}: {1}", System.Net.WebUtility.HtmlEncode(item.Key), System.Net.WebUtility.HtmlEncode(item.Value != null ? item.Value.ToString() : ""));
            ...
            {
                var customer = EngineContextExperimental.Current.Resolve<ICustomerService>().GetCustomerById(order.CustomerId);
        ...
        public virtual void AddShipmentTokens(IList<Token> tokens, Shipment shipment, string languageId)
        {
            ...
            if (!String.IsNullOrEmpty(shipment.TrackingNumber))
            {
                ...
                var shippingService = EngineContextExperimental.Current.Resolve<IShippingService>();
                var _order = EngineContextExperimental.Current.Resolve<IOrderService>().GetOrderById(shipment.OrderId);
            ...
            var order = EngineContextExperimental.Current.Resolve<IOrderService>().GetOrderById(shipment.OrderId);
        ...
        public virtual void AddOrderNoteTokens(IList<Token> tokens, OrderNote orderNote)
        {
            ...
            var order = EngineContextExperimental.Current.Resolve<IOrderService>().GetOrderById(orderNote.OrderId);
        ...
        {
            var productService = EngineContextExperimental.Current.Resolve<IProductService>();
            var orderService = EngineContextExperimental.Current.Resolve<IOrderService>();
        ...
        public virtual void AddCustomerTokens(IList<Token> tokens, Customer customer)
        {
            ...
            //TODO add a method for getting URL (use routing because it handles all SEO friendly URLs)
            string passwordRecoveryUrl = string.Format("{0}passwordrecovery/confirm?token={1}&email={2}", GetStoreUrl(), customer.GetAttribute<string>(SystemCustomerAttributeNames.PasswordRecoveryToken), System.Net.WebUtility.UrlEncode(customer.Email));
            string accountActivationUrl = string.Format("{0}customer/activation?token={1}&email={2}", GetStoreUrl(), customer.GetAttribute<string>(SystemCustomerAttributeNames.AccountActivationToken), System.Net.WebUtility.UrlEncode(customer.Email));
        ...
        {
            var product = EngineContextExperimental.Current.Resolve<IProductService>().GetProductById(productReview.ProductId);
        ...
        {
            var blogPost = EngineContextExperimental.Current.Resolve<IBlogService>().GetBlogPostById(blogComment.BlogPostId);
        ...
        {
            var newsitem = EngineContextExperimental.Current.Resolve<INewsService>().GetNewsById(newsComment.NewsItemId);
        ...
        public virtual void AddAttributeCombinationTokens(IList<Token> tokens, ProductAttributeCombination combination,  string languageId)
        {
            ...
            //that's why we resolve it here this way
            var product = EngineContextExperimental.Current.Resolve<IProductService>().GetProductById(combination.ProductId);
            var productAttributeFormatter = EngineContextExperimental.Current.Resolve<IProductAttributeFormatter>();
        ...
        {
            var customer = EngineContextExperimental.Current.Resolve<ICustomerService>().GetCustomerById(forumPost.CustomerId);
        ...
        {
            var product = EngineContextExperimental.Current.Resolve<IProductService>().GetProductById(subscription.ProductId);
