...
namespace Grand.Services.Tax
{
    ...
    public partial class TaxService : ITaxService
    {
            ...
            bool includingTax, Customer customer, out decimal taxRate)
        {
            ...
            taxRate = decimal.Zero;
            var checkoutAttribute = EngineContextExperimental.Current.Resolve<ICheckoutAttributeService>().GetCheckoutAttributeById(cav.CheckoutAttributeId);
            ...
            out string name, out string address, out Exception exception)
        {
            ...
            try
            {
                ...
                exception = null;
                return /*valid*/false ? VatNumberStatus.Valid : VatNumberStatus.Invalid;
            }
            catch (Exception ex)
            {
