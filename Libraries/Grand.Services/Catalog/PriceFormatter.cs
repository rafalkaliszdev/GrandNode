...
namespace Grand.Services.Catalog
{
    ...
    public partial class PriceFormatter : IPriceFormatter
    {
            ...
            bool showCurrency, Currency targetCurrency)
        {
            ...
            if (showCurrency && false)//_currencySettings.DisplayCurrencyLabel)
            ...
            Currency targetCurrency, Language language, bool priceIncludesTax)
        {
            return FormatPrice(price, showCurrency, targetCurrency, language,
                priceIncludesTax, true);//tbh _taxSettings.DisplayTaxSuffix);
        }
            ...
            Currency targetCurrency, Language language, bool priceIncludesTax, bool showTax)
        {
            ...
            if (showTax)
            {
                ...
                if (priceIncludesTax)
                {
                    formatStr = null;// _localizationService.GetResource("Products.InclTaxSuffix", language.Id, false);
                    if (String.IsNullOrEmpty(formatStr))
                        formatStr = "{0} incl tax";
                }
                else
                {
                    formatStr = null; //_localizationService.GetResource("Products.ExclTaxSuffix", language.Id, false);
                    if (String.IsNullOrEmpty(formatStr))
